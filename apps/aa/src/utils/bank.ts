export const CIPS_BANK = [
        {
            "bankId": "7605",
            "bankName": "Narayani Development Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "NDBLNPKA"
        },
        {
            "bankId": "7518",
            "bankName": "Nepal Infrastucture Bank Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "INFRNPKA"
        },
        {
            "bankId": "7608",
            "bankName": "Salapa Bikas Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "SALAPA"
        },
        {
            "bankId": "9908",
            "bankName": "Corporate Development Bank Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "CORPNPKA"
        },
        {
            "bankId": "9201",
            "bankName": "Shree Investment & Finance Co. Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "SIFLNPKA"
        },
        {
            "bankId": "7604",
            "bankName": "Janaki Finance Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "JANAKI"
        },
        {
            "bankId": "7701",
            "bankName": "Nepal Finance Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "NFLLNPKA"
        },
        {
            "bankId": "7516",
            "bankName": "Best Finance Company Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "GNFLNPKA"
        },
        {
            "bankId": "7607",
            "bankName": "Multipurpose Finance Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "MULTIPKA"
        },
        {
            "bankId": "9902",
            "bankName": "Progressive Finance Co. Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "PFLTDNPKA"
        },
        {
            "bankId": "9906",
            "bankName": "Central Finance Ltd",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "CEFLNPKA"
        },
        {
            "bankId": "6401",
            "bankName": "Sindhu Bikash Bank Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "SINDNPKA"
        },
        {
            "bankId": "7517",
            "bankName": "Green Development Bank Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "GREENNPKA"
        },
        {
            "bankId": "7509",
            "bankName": "Miteri Development Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "MITERIKA"
        },
        {
            "bankId": "9905",
            "bankName": "Pokhara Finance Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "PFLNPKA"
        },
        {
            "bankId": "9915",
            "bankName": "Guheswori Merchant Banking and Finance Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "GMBFNPKA"
        },
        {
            "bankId": "9935",
            "bankName": "Manjushree Finance Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "MAFINPKA"
        },
        {
            "bankId": "8301",
            "bankName": "Gurkhas Finance Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "GURKNPKA"
        },
        {
            "bankId": "9919",
            "bankName": "ICFC Finance Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "ICFCNPKA"
        },
        {
            "bankId": "7502",
            "bankName": "Excel Development Bank Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "AXDLNPKA"
        },
        {
            "bankId": "2301",
            "bankName": "NIC Asia Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "BOALNPKA"
        },
        {
            "bankId": "0801",
            "bankName": "Nepal SBI Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "NSBINPKA"
        },
        {
            "bankId": "2101",
            "bankName": "Prime Commercial Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "PCBLNPKA"
        },
        {
            "bankId": "5401",
            "bankName": "Lumbini Bikas Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "VIBORPKA"
        },
        {
            "bankId": "0101",
            "bankName": "Nepal Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "NEBLNPKA"
        },
        {
            "bankId": "9001",
            "bankName": "Goodwill Finance Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "GFCLNPKA"
        },
        {
            "bankId": "5901",
            "bankName": "Saptakoshi Development Bank Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "SKDBLNKA"
        },
        {
            "bankId": "6601",
            "bankName": "Garima Bikas Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "GRDBLNPKA"
        },
        {
            "bankId": "2201",
            "bankName": "Laxmi Sunrise Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "LXBLNPKA"
        },
        {
            "bankId": "8101",
            "bankName": "Shine Resunga Development Bank Ltd.",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "SRDBNPKA"
        },
        {
            "bankId": "2501",
            "bankName": "NMB Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "NMBBNPKA"
        },
        {
            "bankId": "0301",
            "bankName": "Agricultural Development Bank Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "ADBLNPKA"
        },
        {
            "bankId": "6801",
            "bankName": "Kamana Sewa Bikas Bank Ltd.",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "KAMANAKA"
        },
        {
            "bankId": "1801",
            "bankName": "Siddhartha Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "SIDDNPKA"
        },
        {
            "bankId": "9931",
            "bankName": "Mahalaxmi Bikas Bank Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "MBBLNPKA"
        },
        {
            "bankId": "1601",
            "bankName": "Kumari Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "KMBLNPKA"
        },
        {
            "bankId": "1501",
            "bankName": "Machhapuchchhre Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "MBLNNPKA"
        },
        {
            "bankId": "0201",
            "bankName": "Rastriya Banijya Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "RBBANPKA"
        },
        {
            "bankId": "0601",
            "bankName": "Standard Chartered Bank Nepal Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "SCBLNPKA"
        },
        {
            "bankId": "1001",
            "bankName": "Everest Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "EVBLNPKA"
        },
        {
            "bankId": "7201",
            "bankName": "Muktinath Bikas Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "MNBBLNPKA"
        },
        {
            "bankId": "0401",
            "bankName": "Nabil Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "NARBNPKA"
        },
        {
            "bankId": "1901",
            "bankName": "Global IME Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "GLBBNPKA"
        },
        {
            "bankId": "9939",
            "bankName": "Reliance Finance Ltd.",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "RELFLPKA"
        },
        {
            "bankId": "0501",
            "bankName": "Nepal Investment Mega Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "NIBLNPKT"
        },
        {
            "bankId": "2001",
            "bankName": "Citizens Bank International Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "CTZNNPKA"
        },
        {
            "bankId": "4501",
            "bankName": "Sanima Bank Ltd.",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "SNMANPKA"
        },
        {
            "bankId": "6001",
            "bankName": "Jyoti Bikash Bank Ltd",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "JBBLNPKA"
        },
        {
            "bankId": "0701",
            "bankName": "Himalayan Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "HIMANPKA"
        },
        {
            "bankId": "7301",
            "bankName": "Shangrila Development Bank Limited",
            "selfAccountVerificationFlag": "N",
            "swiftCode": "SGBBNPKA"
        },
        {
            "bankId": "2601",
            "bankName": "Prabhu Bank Limited",
            "selfAccountVerificationFlag": "Y",
            "swiftCode": "PRVUNPKA"
        }
    ]

const getBankId = (bankName: string): string | null => {
    const bankId = CIPS_BANK.find((bank) => bank.bankName.toLocaleLowerCase() === bankName.toLocaleLowerCase())?.bankId;
    return bankId || null;
}

const getBankName = (bankId: string): string | null => {
    const bankName = CIPS_BANK.find((bank) => bank.bankId.toLocaleLowerCase() === bankId.toLocaleLowerCase())?.bankName;
    return bankName || null;
}

export { getBankId, getBankName };