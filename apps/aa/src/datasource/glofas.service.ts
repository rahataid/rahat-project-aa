...
export class GlofasService implements AbstractSource {
  ...
  async criteriaCheck(payload: AddTriggerStatement) {
    ...
    if (triggerData.isTriggered) {
      this.logger.log(`Trigger with repeat key ${triggerData.repeatEvery} already triggered.`)
    ...
    const recentStationData = JSON.parse(
      JSON.stringify(recentData.data)
    ) as GlofasDataObject;
  ...
  parseReturnPeriodTable(
    rpTable: cheerio.Cheerio<cheerio.Element>,
    $: cheerio.CheerioAPI
  ) {
  ...
  parsePointForecast(
    pfTable: cheerio.Cheerio<cheerio.Element>,
    $: cheerio.CheerioAPI
  ) {
