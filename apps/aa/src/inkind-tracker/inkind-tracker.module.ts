...
import { InkindTrackerController } from './inkind-tracker.controller';
import { InkindTrackerService } from './inkind-tracker.service';
import { SettingsService } from '@rumsan/settings';
import { PrismaModule } from '@rumsan/prisma';

@Module({
  imports: [PrismaModule],
  controllers: [InkindTrackerController],
  providers: [InkindTrackerService],
  exports: [InkindTrackerService],
})
export class InkindTrackerModule implements OnModuleInit, OnModuleDestroy {
  constructor(
    private readonly inkindTrackerService: InkindTrackerService,
  ...
  async onModuleInit() {
    try {
      ...
      await this.inkindTrackerService.initialize({
      ...
      console.log('inkind Tracker Module initialized successfully');
    } catch (error) {
      console.error('Failed to initialize inkind Tracker Module:', error);
  ...
  async onModuleDestroy() {
    try {
      await this.inkindTrackerService.cleanup();
      console.log('inkind Tracker Module cleaned up successfully');
    } catch (error) {
      console.error('Failed to cleanup inkind Tracker Module:', error);
    }
  }
  ...
  private async loadEntitiesFromDatabase() {
    try {
      ...
      await this.inkindTrackerService.loadEntities(transformedEntities);
      console.log(
