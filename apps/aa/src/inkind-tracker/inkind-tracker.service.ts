...
export interface InkindTrackerConfig {
  network: {
    rpcUrl: string;
    entryPoint: string;
  };
...
@Injectable()
export class InkindTrackerService {
  private readonly logger = new Logger(InkindTrackerService.name);
  ...
  /**
   * Initialize inkind tracker with configuration from database
   */
  async initialize(config: InkindTrackerConfig): Promise<void> {
    try {
