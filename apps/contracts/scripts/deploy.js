...
async function main() {
  ...
  const aaVoucher = await ethers.deployContract('RahatToken', [
    forwarderAddress,
    'aaVoucher',
    'AA',
    donorAddress,
  ...
  const aaProject = await ethers.deployContract('AAProject', [
    'AAProject',
    aaVoucherAddress,
    forwarderAddress,
    accessManagerAddress,
  ...
  console.log('-----add donor as token owner---');
  await donorContract.addTokenOwner(aaVoucherAddress, donorAddress);

  // !!!! todo: Remove after test
  console.log('-----mint initial tokens to project---: Remove after test');
  // Mint 1,000,000 tokens (with 1 decimal, this is 1,000,000 * 10^1 = 10,000,000)
  const initialTokenAmount = ethers.parseUnits('1000000', 1); // 1 million tokens with 1 decimal
  await donorContract.mintTokens(
    aaVoucherAddress,
    aaProjectAddress,
    initialTokenAmount
  );
  console.log(
    `Minted ${ethers.formatUnits(initialTokenAmount, 1)} tokens to project`
  );
